...
elseif ($_GET[module]=='detailproduk'){
	...
	$detail=mysql_query("SELECT * FROM produk,subkat    
                      WHERE subkat.subkat_id=produk.id_kategori 
	...
	echo "<h4 class='heading colr'>Kategori: <a href='kategori-$d[subkat_id]-$d[subkat_seo].html'>$d[subkat_nama]</a></h4></div>";
	...
	echo"<div class='feat_prod_box_details'>";
 	if ($d['gambar']!=''){ 
		echo "<div class='prod_img3'><a href='foto_produk/$d[gambar]' rel='clearbox[gallery=Koleksi Produk]' title='$d[nama_produk]'><img src='foto_produk/".$d['gambar']."' width=180  class='tooltip' title='klik untuk memperbesar gambar' border='0' rel='clearbox[gallery=Koleksi Produk]' title='$d[nama_produk]'/></a>
				...
				//echo "<img src='foto_produk/".$d['gambar']."'>";
...
elseif ($_GET[module]=='ikategori'){
  ...
  // Tampilkan nama kategori
  $isq = mysql_query("SELECT kategori.nama_kategori, 
  							 kategori.id_kategori, 
							 kategori.kategori_seo, 
							 kategori.gambar_kat, 
                             COUNT(produk.id_produk) AS jml 
                             FROM kategori 
							 LEFT JOIN produk ON produk.id_kategori=kategori.id_kategori 
							 WHERE kategori_induk='$_GET[id]' GROUP BY nama_kategori");
	//echo "SELECT * FROM kategori WHERE  kategori_induk='$_GET[id]' ";
  $on = mysql_num_rows($isq);
  	if ($on=='') {
	  //echo "Belum Ada Produk Pada Kategori Ini";
	  $sub = mysql_query("SELECT subkat.subkat_nama, 
  							 subkat.subkat_id, 
							 subkat.subkat_seo, 
							 subkat.subkat_gambar, 
                             COUNT(produk.id_produk) AS jml 
							 FROM subkat 
							 LEFT JOIN produk ON produk.id_kategori=subkat.subkat_id 							 
							 WHERE subkat.id_kategori='$_GET[id]' GROUP BY subkat_nama");
	  while ($isub = mysql_fetch_array($sub)) {
  		echo "<div class='prod_box'>
		 <div class='top_prod_box'></div>
		 <div class='center_prod_box'>            
             <div class='product_title'>
		<a href='kategori-$isub[subkat_id]-$isub[subkat_seo].html'><h4>$isub[subkat_nama]</h4></a>
			</div>
             <div class='product_img'>
			 <a href='kategori-$isub[subkat_id]-$isub[subkat_seo].html'>
			 <img src='foto_kategori/$isub[subkat_gambar]' border='0' height=110 width=114  class='tooltip' title='klik membuka kategori'>
			 </a>
			 </div>
			 <div class='prod_price'>
			 <span class='price'>$isub[jml] Item</span>
			 </div>
             </div>
		</div>";
  	}
	  ////////////////////////// ambil dari subkat end //////////////////////////
  	}else{
  		while ($in = mysql_fetch_array($isq)) {
  		echo "<div class='prod_box'>
		 <div class='top_prod_box'></div>
		 <div class='center_prod_box'>            
             <div class='product_title'>
		<a href='ikategori-$in[id_kategori]-$in[kategori_seo].html'><h4>$in[nama_kategori]</h4></a>
			</div>
             <div class='product_img'>
			 <a href='ikategori-$in[id_kategori]-$in[kategori_seo].html'>
			 <img src='foto_kategori/$in[gambar_kat]' border='0' height=110 width=114  class='tooltip' title='klik membuka kategori'>
			 </a>
			 </div>
			 <div class='prod_price'>
			 <!---span class='price'>$in[jml] Item</span---->
			 </div>
             </div>
		</div>";
  	}
...
elseif ($_GET[module]=='subkategori'){
  ...
  $isq = mysql_query("SELECT subkat.subkat_nama, 
  							 subkat.subkat_id, 
							 subkat.subkat_seo, 
							 subkat.subkat_gambar, 
                             COUNT(produk.id_produk) AS jml 
  ...
  $on = mysql_num_rows($isq);
	  ...
	  echo "Belum Ada Produk Pada Kategori Ini";
		 ...
		 <div class='center_prod_box'>            
             <div class='product_title'>
		<a href='kategori-$in[id_kategori]-$in[subkat_seo].html'><h4>$in[subkat_nama]</h4></a>
			</div>
             <div class='product_img'>
			 <a href='kategori-$in[id_kategori]-$in[kategori_seo].html'>
			 <img src='foto_kategori/$in[gambar_kat]' border='0' height=110 width=114  class='tooltip' title='klik membuka kategori'>
			 </a>
			 </div>
			 <div class='prod_price'>
...
elseif ($_GET[module]=='detailkategori'){
  // Tampilkan nama kategori
  $sq = mysql_query("SELECT subkat_nama from subkat where subkat_id='$_GET[id]'");
  $n = mysql_fetch_array($sq);

  echo "<h4 class='heading colr'>Kategori: $n[subkat_nama]</h4></div>";
  ...
  // Tampilkan daftar produk yang sesuai dengan kategori yang dipilih //sebelumnya pake id_kategori
 	$sql = mysql_query("SELECT * FROM produk WHERE id_kategori='$_GET[id]' 
            ORDER BY id_produk DESC LIMIT $posisi,$batas");	
	//echo "SELECT * FROM produk WHERE id_kategori='$_GET[id]' 
...
if ($_GET['module']=='selesaibelanja'){
  ...
  $ketemu=mysql_num_rows($sql);
  if($ketemu < 1){
   echo "<script> alert('Keranjang belanja masih kosong');window.location='store'</script>\n";
   	 exit(0);
	}
	else{
