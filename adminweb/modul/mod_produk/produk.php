...
switch($_GET[act]){
  ...
  echo '<SCRIPT LANGUAGE="javascript">
    ...
    var baseURL = "media.php?module=produk&kategori="
    ...
    echo "<h2>Tambah Produk</h2>
		  ...
		  <select name=\"kategori\" id=\"kategori\" onchange=\"OnChange(this.form.kategori);\">
		   <option value=0 selected>- Pilih Kategori -</option>";
            $tampil=mysql_query("SELECT * FROM subkat ORDER BY subkat_nama");
            while($r=mysql_fetch_array($tampil)){
              echo "<option value=$r[subkat_id]>$r[subkat_nama]</option>";
            }
echo"		  </select>
			</form>
			<br>
...
echo"		  </select>
    ...
    $tampil = mysql_query("SELECT * FROM produk where id_kategori=$_GET[kategori] ORDER BY id_produk DESC LIMIT $posisi,$batas");
    ...
    $jmldata = mysql_num_rows(mysql_query("SELECT * FROM produk where id_kategori=$_GET[kategori]"));
    $jmlhalaman  = $p->jumlahHalaman($jmldata, $batas);
    $linkHalaman = $p->navHalaman($_GET[halaman], $jmlhalaman);
  ...
  case "tambahproduk":
    ...
    echo "</select></td></tr>
          ...
          <tr><td>Deskripsi</td>  <td> <textarea name='deskripsi' id='loko' style='width: 600px; height: 350px;'></textarea></td></tr>
  ...
  case "editproduk":
    ...
    echo "<h2>Edit Produk</h2>
          ...
          while($w=mysql_fetch_array($tampil)){
            if ($r[id_kategori]==$w[subkat_id]){
              echo "<option value=$w[subkat_id] selected>$w[subkat_nama]</option>";
            }
            else{
              echo "<option value=$w[subkat_id]>$w[subkat_nama]</option>";
            }
          }
    echo "</select></td></tr>
          ...
          <tr><td>Deskripsi</td>   <td> <textarea name='deskripsi' id='loko' style='width: 600px; height: 350px;'>$r[deskripsi]</textarea></td></tr>
          <tr><td>Gambar</td>       <td> :  
